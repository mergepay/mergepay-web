...
export function AssetSwitcher({
  ...
  const [modalAsset, setModalAsset] = useState<TrustlineAsset | null>(null);
  ...
  const handleOpenTrustlineModal = (asset: TrustlineAsset) => {
    setModalAsset(asset);
  };
    ...
    <>
      <Card className={`overflow-hidden border-3 border-ink bg-cream shadow-brutal ${className}`}>
        ...
        <CardContent className="p-5 space-y-5">
          ...
          {!selectedAssetItem.hasTrustline && selectedAssetItem.issuer && (
            <div
            ...
            >
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => handleOpenTrustlineModal(selectedAssetItem)}
                className="shrink-0 bg-cream text-xs min-h-[38px] font-bold border-2 border-ink"
              >
      ...
      {modalAsset && modalAsset.issuer && (
        <TrustlineModal
          open={Boolean(modalAsset)}
          onClose={() => setModalAsset(null)}
          assetCode={modalAsset.code}
          assetIssuer={modalAsset.issuer}
          assetName={modalAsset.name}
          accountPublicKey={user?.stellarPublicKey}
          onSuccess={() => {
            refetch();
          }}
        />
