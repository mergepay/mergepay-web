...
export default function DashboardPage() {
  ...
  // Balances are per-asset: a USDC credit and an XLM debt cannot be added
  // into one figure without printing a number that means nothing. The
  // cards lead with the asset carrying the most exposure and the rest are
  // listed underneath, each with its own label.
  const totalsByAsset = useMemo(() => summarizeNetsByAsset(groups), [groups]);
  const primaryTotals = totalsByAsset[0];
  const otherTotals = totalsByAsset.slice(1);
  // With no groups yet there is no asset in play, so the zeroes are shown
  // without a label rather than being attributed to an arbitrary asset.
  const primaryAsset = primaryTotals?.assetCode;
